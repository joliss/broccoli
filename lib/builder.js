...
function Builder(node, options) {
  ...
  for (var i = 0; i < this.builderNodes.length; i++) {
    ...
    this.builderNodes[i].description = formatNode(this.builderNodes[i])
  }

  this.setupTmpDirs()
  // TODO: wrap this in try ... finally, so we always call cleanup
  this.setupNodes()
...
Builder.prototype.makeBuilderNode = function(node, _stack) {
  ...
  // Make sure that `node` is in fact a Broccoli node
