...
// TODO: "thrown from"
// TODO turn timings into milliseconds?
      ...
      promise = promise.then(function() {
        ...
        return RSVP.resolve()
          ...
          .finally(function() {
            ...
            bn.lastBuild.selfTime = 1000 * ((now[0] - startTime[0]) + (now[1] - startTime[1]) / 1e9)
...
Builder.prototype.makeBuilderNode = function(node, _stack) {
  ...
  if (node == null || !node.__broccoliGetInfo__) {
    ...
    if (_stack.length > 0) {
      ...
      throw new InvalidNodeError(message + ' as root node')
    }
  }
...
BuilderNode.prototype.toString = function() {
  ...
  if (this.pluginInterface.nodeType === 'transform') {
    ...
    if (this.lastBuild) {
      hint += ' (' + Math.round(this.lastBuild.selfTime) + ' ms)'
    }
  } else { // nodeType === 'source'
