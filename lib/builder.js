...
function Builder(node, options) {
  ...
  // nodeWrappers are wrappers around nodes storing additional information.
  // This array contains them sorted in topological (build) order.
  this.nodeWrappers = []
  // This populates this.nodeWrappers as a side effect
  this.nodeWrapper = this.makeNodeWrapper(this.node)
  ...
  try {
    ...
    this.setupNodes()
    this.outputPath = this.nodeWrapper.outputPath
    ...
    if (nw.nodeInfo.nodeType === 'transform') {
      ...
      promise = promise.then(function() {
        ...
        return RSVP.resolve()
          ...
          .finally(function() { self.trigger('nodeEnd', nw) })
      })
    } else { // nodeType === 'source'
      promise = promise.then(function() { self.trigger('nodeEnd', nw) })
...
Builder.prototype.setupNodes = function() {
  for (var i = 0; i < this.nodeWrappers.length; i++) {
    ...
    if (nw.nodeInfo.nodeType !== 'transform') continue
    try {
