...
module.exports = function getMiddleware(watcher, options) {
  ...
  return function broccoliMiddleware(request, response, next) {
    watcher.currentBuild.then(function() {
      ...
      // Most errors will be Builder.BuildError objects, but in principle we
      // need to deal with random error objects here, because

      // In principle, we need to deal with random error objects here, because the
      var message = buildError
